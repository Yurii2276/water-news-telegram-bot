...
test("unregistered Google News source can reach AI classification after full extraction", async () => {
  const item = {
    title: "Міська інфраструктура потребує оновлення",
    url: "https://news.google.com/rss/articles/example",
    sourceId: "google_news",
    sourceName: "Нове локальне медіа",
    discoveryMethod: "google_news",
  };
  const saved = [];
  const pipeline = createEditorPipeline({
    ...
    extract: async () => ({
      ...item,
      url: "https://new-media.example/water-project",
      title: "Громада модернізує систему централізованого водопостачання",
      content: "Громада повідомила про модернізацію системи централізованого водопостачання. ".repeat(8),
      extractionStatus: "ok",
      sourceTrusted: false,
      contextBasis: "full_article",
    }),
    classify: async () => ({
      ...
      confidence: "high",
      confidenceScore: 90,
      summary: "Громада модернізує систему централізованого водопостачання.",
      whyImportant: "Оновлення стосується водної інфраструктури громади.",
      hashtags: ["#вода"],
    }),
    repository: {
      listForDedup: async () => [],
      saveMaterial: async (material) => {
        const stored = { id: 1, ...material };
        saved.push(stored);
        return stored;
      },
    },
