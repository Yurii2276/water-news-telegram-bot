...
export function formatScanReport(report) {
  ...
  const lines = [
    ...
    `Google News збоїв: ${report.google_failures ?? 0}`,
    `Google degraded mode: ${report.google_degraded_mode ? "так" : "ні"}`,
    `Bing News fallback запитів: ${report.bing_queries_executed ?? 0}`,
    `Bing fallback успішно: ${report.bing_fallback_successes ?? 0}`,
