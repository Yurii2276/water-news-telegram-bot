  ...
  return async function handleUpdate(update) {
    ...
    if (command === "/queue") {
      ...
      if (materials.length === 0) {
        await telegram.sendMessage(chatId, "Черга автопублікації порожня.");
      } else {
    ...
    if (command === "/news") {
      ...
      if (materials.length === 0) {
        await telegram.sendMessage(chatId, "Опублікованих матеріалів ще немає.");
      } else {
        for (const material of materials) {
          await telegram.sendMessage(chatId, formatPublication(material));
        }
      }
      return;
    }
